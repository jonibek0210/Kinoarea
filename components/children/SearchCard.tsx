...
const SearchCard: React.FC<SearchProps> = ({ item, type }) => {
	...
	return (
		<Link href={{ pathname: type == 'person' ? `/actors/${item.id}` : type == 'movie' ? `/movies/${item.id}` : `/series/${item.id}`, query: { id: item.id } }} className="h-fit flex justify-between items-center p-3 relative rounded-lg bg-[#1E2538]" >
			<div className="h-full flex max-md:flex-col items-center max-md:justify-between w-full gap-5">
				<div className="overflow-hidden rounded-lg w-1/6 max-lg:w-2/6 max-md:w-full">
					<img className='w-full h-full object-cover' src={type === 'person' ? imgPeople : imgMovie} alt={item?.title || item?.name} />
				</div>
				<div className="">
					<p className='text-3xl max-lg:text-xl max-md:text-sm || font-bold text-white max-md:text-center'>{item?.title || item?.name}</p>
